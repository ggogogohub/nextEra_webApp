...
import { User as UserIcon, Settings, LogOut, User, Bell, Moon, ChevronRight } from 'lucide-react';
...
const Status = styled.div<{ status: 'online' | 'busy' | 'away' | 'offline' }>`
  position: absolute;
  bottom: -2px;
  right: -2px;
  width: 12px;
    ...
    <AvatarContainer>
      ...
      >
        {imageUrl ? (
          ...
          <UserIcon color="#fff" size={22} strokeWidth={1.5} />
        )}
      </AvatarCircle>
      
      <Status status={status} />
      
      <ProfileMenu isOpen={isOpen} ref={menuRef}>
        <ProfileHeader>
          <ProfileImage>
            {imageUrl ? (
              <img src={imageUrl} alt={name} />
            ) : (
              <UserIcon color="#fff" size={32} strokeWidth={1.5} />
            )}
          </ProfileImage>
